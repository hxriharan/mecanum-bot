...
const uint8_t LF_DIR = 22; // previously 19
const uint8_t LF_PWM = 23; // previously 21

const uint8_t RF_DIR = 19; // previously 22
const uint8_t RF_PWM = 21; // previously 23

const uint8_t LB_DIR = 14; // previously 18
const uint8_t LB_PWM = 27; // previously 17

const uint8_t RB_DIR = 18; // previously 14
const uint8_t RB_PWM = 17; // previously 27 then 17 *CHANGED TO 27 to Test it out*
...
void loopMotor(){
	if (millis() - LF_last_update > 1000/motor_rate && LF_pwm != _LF_pwm)
	{
		...
		_LF_pwm = LF_pwm;
		digitalWrite(LF_DIR, _LF_pwm > 0 ? LOW : HIGH);
